...
| F7.10 | `getCurrentPosition` is called without `enableHighAccuracy`, so the browser answers from whatever source is cheapest — on a desktop that is an IP lookup, which resolves to the ISP's registered city. Reported from Indore, it returned Bhopal (~190 km out), and the 5 km search then finds centres in the wrong city. `coords.accuracy`, the one value that reveals this, was discarded. | `enableHighAccuracy: true`, `maximumAge: 0` and a 20s timeout so GPS/Wi-Fi is actually attempted. The fix's accuracy is now kept and, above 5 km, the UI says the position came from the internet provider rather than GPS and is likely the wrong city. Since no flag can fix desktop IP geolocation, the map is now click-to-set: clicking re-runs the search around that point. The map also appears when geolocation fails outright, so a denied permission is no longer a dead end. | [x] |
